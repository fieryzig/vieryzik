...
_querySwapchainSupport(vk::PhysicalDevice device, vk::SurfaceKHR surface)
{
...
template<typename T>
T
choose_by_cond(std::vector<T> Ts, std::function<bool(T)> cond, T legacy)
{
  for (const T& t : Ts) {
    if (cond(t)) {
      return t;
...
vk_swapchain*
vk_swapchain_create(vk_device* device, vk_surface* surface)
{
  auto ret = new vk_swapchain();
  auto supportDetails =
    _querySwapchainSupport(device->physicalDevice, surface->surface);

  auto surfaceFormat = choose_by_cond<vk::SurfaceFormatKHR>(
  ...
  auto presentMode = choose_by_cond<vk::PresentModeKHR>(
    supportDetails.presentModes,
    [](auto each) {
      if (each == vk::PresentModeKHR::eMailbox)
        return true;
      return false;
    },
